# People fetch – single person – error flows

```mermaid
flowchart TD
  A[GET /api/people/:tmdbPersonId] --> B{Valid tmdbPersonId?}
  B -- No --> E400[400 Bad Request\nVALIDATION_ERROR]

  B -- Yes --> C{Cache hit and fresh?}
  C -- Yes --> OK200[200 OK\nPersonResponseDTO]

  C -- No --> D[Call TMDB: GET /person/:tmdbPersonId]
  D --> F{TMDB response}
...
Returned when `tmdbPersonId` is not a positive integer.

### 404 PERSON_NOT_FOUND
Returned when TMDB responds with 404 for the given `tmdbPersonId`.

### 502 UPSTREAM_TMDB_ERROR
Returned when TMDB is unavailable, times out, or returns 5xx.

### 502 UPSTREAM_SCHEMA_MISMATCH
Returned when TMDB returns 200 but the payload does not match `TmdbPersonRaw` schema.
